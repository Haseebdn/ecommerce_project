...
if (isset($_SESSION['role_id'])) {
    ...
    $query = "SELECT * FROM `admin_role` WHERE `id`=$roleId";
    $fet = mysqli_query($conn, $query);
    $role = mysqli_fetch_assoc($fet);
    // print_r($role);
}
?>
...
<div class="main-sidebar sidebar-style-2">
    <aside id="sidebar-wrapper">
        ...
        <ul class="sidebar-menu mb-4">
            ...
            <!-- Categories -->
            <?php
            $catPages = ['cat_form.php', 'cat_table.php'];
            if (@$role['role_type'] == "All" || @$role['categories'] == 1) {
            ?>
            ...
            <?php
            $subcatPages = ['subcat_form.php', 'subcat_table.php'];
            if (@$role['role_type'] == "All" || @$role['subcategories'] == 1) {
            ?>
            ...
            <!-- Suppliers -->
            <?php
            $suppPages = ['supplier_form.php', 'supplier_table.php'];
            if (@$role['role_type'] == "All" || @$role['suppliers'] == 1) {
            ?>
            ...
            <!-- Quantity Units -->
            <?php
            $qtyPages = ['qtyUnit_form.php', 'qtyUnit_table.php'];
            if (@$role['role_type'] == "All" || @$role['qty_units'] == 1) {
            ?>
            ...
            <!-- Products -->
            <?php
            $productPages = ['product_form.php', 'product_table.php'];
            if (@$role['role_type'] == "All" || @$role['products'] == 1) {
            ?>
            ...
            ?>
                <li class="dropdown <?php echo isActive($userPages, $page) ? 'active' : ''; ?>">
                    ...
                    <ul class="dropdown-menu" style="<?php echo isActive($userPages, $page) ? 'display:block;' : ''; ?>">
                        ...
                        <li class="<?php echo ($page == 'pos_view.php') ? 'active' : ''; ?>">
                            <a class="nav-link" href="pos_view.php">POS View</a>
                        </li>
            ...
            if (@$role['role_type'] == "All" || @$role['adm_manage'] == 1) {
            ?>
                <li class="dropdown <?php echo isActive($userPages, $page) ? 'active' : ''; ?>">
                    <a href="#" class="menu-toggle nav-link has-dropdown">
